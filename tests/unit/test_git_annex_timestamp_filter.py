...
def test_filter_no_changes(datalad_repo: Path) -> None:
    """Test _filter_timestamp_only_changes with no changes returns False."""
    service = GitAnnexService(datalad_repo)

    # Create and commit a file
    test_file = datalad_repo / "metadata.json"
    test_file.write_text(json.dumps({"video_id": "test123", "fetched_at": "2026-01-01T00:00:00"}))
    subprocess.run(["git", "add", "metadata.json"], cwd=datalad_repo, check=True)
    subprocess.run(["git", "commit", "-m", "Initial commit"], cwd=datalad_repo, check=True, capture_output=True)
...
@pytest.mark.ai_generated
def test_filter_timestamp_only_changes_detected(datalad_repo: Path) -> None:
    ...
    diff_result = subprocess.run(["git", "diff", "--name-only"], cwd=datalad_repo, capture_output=True, text=True)
    assert diff_result.stdout.strip() == "", "File should be restored with no changes"


@pytest.mark.ai_generated
def test_filter_real_changes_preserved(datalad_repo: Path) -> None:
...
@pytest.mark.ai_generated
def test_filter_mixed_files(datalad_repo: Path) -> None:
    """Test _filter_timestamp_only_changes with mix of timestamp-only and real changes."""
    service = GitAnnexService(datalad_repo)

    # Create and commit two files
    file1 = datalad_repo / "video1" / "metadata.json"
    file2 = datalad_repo / "video2" / "metadata.json"
    ...
    subprocess.run(["git", "add", "."], cwd=datalad_repo, check=True)
    subprocess.run(["git", "commit", "-m", "Initial commit"], cwd=datalad_repo, check=True, capture_output=True)
...
@pytest.mark.ai_generated
def test_filter_tsv_last_updated_column(datalad_repo: Path) -> None:
    """Test _filter_timestamp_only_changes handles TSV last_updated column."""
    service = GitAnnexService(datalad_repo)

    # Create and commit TSV file
    tsv_file = datalad_repo / "videos.tsv"
    ...
    subprocess.run(["git", "add", "videos.tsv"], cwd=datalad_repo, check=True)
    subprocess.run(["git", "commit", "-m", "Initial commit"], cwd=datalad_repo, check=True, capture_output=True)
...
@pytest.mark.ai_generated
def test_filter_playlist_last_modified(datalad_repo: Path) -> None:
    """Test _filter_timestamp_only_changes handles playlist last_modified field."""
    service = GitAnnexService(datalad_repo)

    # Create and commit playlist metadata
    playlist_file = datalad_repo / "playlists" / "PL123" / "metadata.json"
