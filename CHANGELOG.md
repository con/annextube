...
- **Video descriptions are searchable** (#6, FR-042d–FR-042g): searching a speaker's name that appears only in a talk's description used to return nothing, because `videos.tsv` carried no descriptions and the client-side index had nothing to match. `videos.tsv` now has a `description` column with the first non-empty line, and any description that does not fit that line is exported to `videos/video_fulldescriptions.json`, which the web UI loads alongside the TSV. The file follows the archive's normal `.gitattributes` rules (a large one is annexed like any other file, so its content needs depositing); when it is missing or its annexed content was never deposited the UI logs a warning and falls back to the first line
- **Pagefind index covers video metadata, not just captions** (#6, FR-042f): one record per video (title, description, tags) tagged with `record_type`, so videos **without** captions are findable in full-text search; index rebuilds are now also triggered by `metadata.json` changes, not only caption changes
- **Search modes renamed "Metadata" and "Full"** (#6, FR-042g): "Metadata" is the offline-capable fuse.js search over titles, descriptions and tags; "Full" is the Pagefind search over metadata **and** caption text. Description matches carry a `description` badge instead of a timestamp. Previously shared `mode=captions` links keep working
- **`generate-web` warns when a development checkout's bundle is stale** (#6): `web/` is git-ignored and built at install time, so checking out a branch that changes `frontend/src/` and running `generate-web` without rebuilding silently deployed the *previous* UI — a whole feature could be missing from the archive with no diagnostic. The command now names the newer sources and points at `npm run build`

### Fixed

- **Search excerpts cannot inject markup** (#6): Pagefind excerpts are built from YouTube-supplied descriptions and captions and are rendered with `{@html}`. Pagefind escapes tag delimiters itself, so no payload was executing, but the UI relied on that behaviour; excerpts are now sanitized where they enter the app — everything but Pagefind's own `<mark>` highlights is escaped — so markup in a description can never become an element regardless of what the indexer does
- **A description quoting an annex path is no longer discarded** (#6): `video_fulldescriptions.json` was rejected whenever its bytes contained `/annex/objects/`, which also matched a genuine file whose description text mentions such a path. The undeposited-annex-content case is now recognised by the JSON parse failing, which a real pointer always does
- **Shared links with filters did not restore on channel pages** (#6): `URLStateManager.parseHash` stripped only a leading `#/` and parsed the rest as a query string, so on any non-root route the whole `route?param` collapsed into a single key and every filter parameter was silently dropped — a link like `#/channel/<dir>?search=<term>` opened with an empty search box and the unfiltered list. Query parameters are now parsed from the first `?`, matching the video-route parser
- **Caption search states plainly when only approximate matches exist** (#5): Pagefind never returns zero results for a word it half-knows — searching "reprostim" against an index that only contains "repro" returned every "repro" caption presented as ordinary matches. Results whose per-word match scores are all below the genuine-match tier are now flagged, and when every result is approximate the header reads "No captions or descriptions contain '<query>' — showing approximate matches only"; in mixed result sets genuine matches are listed first and approximate ones carry an "approximate" badge, with their count noted in the header. Stemmed matches ("pizzas" → "pizza") and prefix queries ("datala" → "datalad") are unaffected
