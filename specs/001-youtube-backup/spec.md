...
- **FR-042d**: Metadata search MUST match against full video descriptions (loaded in one additional request alongside `videos.tsv`) and MUST degrade gracefully — falling back to the TSV first-line column, with a logged warning — when the file is absent, unreadable, or served as an annex pointer whose content was not deposited
