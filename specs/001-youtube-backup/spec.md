...
- **FR-039a**: Web interface MUST revalidate all dynamic data files (videos.tsv, video_fulldescriptions.json, metadata.json, channels.tsv, channel.json, per-channel videos.tsv and playlists.tsv) on every session load by fetching with `{ cache: 'no-cache' }`, so that a browser which has a cached copy still sends a conditional request (ETag / If-Modified-Since) and receives fresh content after an archive update without requiring the user to perform a hard reload. Exploratory / optional fetches (HEAD probes, comments.json) are exempt from this requirement. Implementation: pass the `cache: 'no-cache'` option to every `fetch()` call for the data files listed above in `data-loader.ts`
