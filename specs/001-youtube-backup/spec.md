...
6. **Given** a Metadata search where some results are Fuse.js fuzzy-only matches, **When** results are displayed, **Then** exact matches (query is a literal substring of title / description / channel / tag) appear before approximate ones, separated by a visible "Approximate matches" divider, and the result-count header states how many of each kind were found
...
- **FR-039a**: Web interface MUST revalidate all dynamic data files (videos.tsv, video_fulldescriptions.json, metadata.json, channels.tsv, channel.json, per-channel videos.tsv and playlists.tsv) on every page mount (once per session per file, via HTTP conditional request) by fetching with `{ cache: 'no-cache' }`, so that a browser which has a cached copy still sends an ETag / If-Modified-Since request and receives fresh content after an archive update without requiring the user to perform a hard reload. Exempt fetches (no `cache: 'no-cache'` needed): HEAD probes for archive-root detection, `comments.json` (optional per-video detail loaded on demand), and `playlist.json` (optional per-playlist detail loaded on demand). Implementation: pass the `cache: 'no-cache'` option to every `fetch()` call for the data files listed above in `data-loader.ts`
...
- **FR-042c**: Caption search MUST distinguish genuine matches from Pagefind's approximate fallback (when a query word is absent, Pagefind matches a shorter indexed word — e.g. "reprostim" matching only "repro") and MUST state when results are approximate rather than presenting them as matches: genuine results listed first, approximate ones badged, and an explicit "no captions or descriptions contain <query>" statement when no result matches genuinely (the statement names descriptions too, since Full mode searches metadata records as well as captions). Stemmed matches ("pizzas" → "pizza") and prefix queries ("datala" → "datalad") count as genuine. A record that matches with no word-level score data at all MUST be treated as genuine only when the scores are absent because the index predates them; a record reporting an empty score list matched no words and MUST count as approximate, so a video matched solely through metadata does not outrank genuine near misses
- **FR-042d**: `videos.tsv` MUST include a `description` column containing the first non-empty line of each video's description; complete descriptions that do not fit that single line MUST be exported to `videos/video_fulldescriptions.json` (a `{video_id: full description}` lookup, per channel directory). The file follows the archive's normal `.gitattributes` rules (large ones are annexed and their content deposited like any other archive file), and MUST NOT be written when no video needs an entry
- **FR-042e**: Metadata search MUST match against full video descriptions (loaded in one additional request alongside `videos.tsv`) and MUST degrade gracefully — falling back to the TSV first-line column, with a logged warning — when the file is absent, unreadable, or served as an annex pointer whose content was not deposited
- **FR-042f**: The Pagefind index MUST contain one metadata record per video (title, description, tags), with all records tagged by `record_type` (`metadata`/`caption`), so videos without captions are findable in Full mode; metadata changes MUST trigger incremental index rebuild (not only caption changes)
- **FR-042g**: Search UI MUST present the two modes as "Metadata" and "Full"; previously shared URLs using `mode=captions` MUST keep working (mapped to Full mode)
- **FR-042h**: Metadata search (Fuse.js) MUST classify each result as exact (query is a literal case-insensitive substring of any searchable field: title, channel_name, description, or any tag) or approximate (fuzzy match only). Exact results MUST be displayed before approximate results; a full-width visual separator labelled "Approximate matches" MUST be injected at the boundary when both groups are non-empty. The result-count header MUST use unified phrasing: "All N videos match" when search is active and all videos in the archive are returned as exact hits; "Showing N exact hit(s) from T videos" when only exact results are present but some videos are filtered out; "Showing N exact + M approximate hits from T videos" when both groups are non-empty. When no search is active, existing phrasing ("Showing N of T videos" for filtered views, "T videos" otherwise) is unchanged. Implementation: `isExact: boolean` on `SearchResult`; `fuzzyStartIndex` bindable prop threaded from `FilterPanel` → `App` → `VideoList`; each group sorted independently by the active sort preference before concatenation
...
- **FR-047a**: Web interface MUST offer ready-to-copy clone commands for the archive being browsed, in both DataLad (`datalad clone`) and Git/git-annex (`git clone`) form, together with a `get` command for the video on screen. The commands MUST address the archive itself: the repository is located by probing for an HTTP-reachable `.git/` under the *discovered archive root* (FR-039a's root detection), never relative to the web server root, which may host an unrelated repository -- e.g. `https://datasets.datalad.org/.git` exists and is not the archive published at `https://datasets.datalad.org/<org>/<collection>/`. Every path in a `get` command MUST be relative to the repository that the clone command beside it creates, and a `get` command MUST be omitted rather than guessed when the tool it is written for cannot reach the video's location. When no `.git/` is reachable (file:// browsing, an archive published without its git directory), the interface MUST offer nothing rather than a speculative URL. Implementation: `frontend/src/services/git-discovery.ts` probes `<root>/.git/HEAD` with a HEAD request; `frontend/src/components/CloneCommand.svelte` renders the panel
